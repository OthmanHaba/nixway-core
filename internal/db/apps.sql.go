...
const updateAppRegistryCredential = `-- name: UpdateAppRegistryCredential :one
UPDATE apps
...
type UpdateAppRegistryCredentialParams struct {
	ID                   uuid.UUID   `json:"id"`
	RegistryCredentialID pgtype.UUID `json:"registry_credential_id"`
}

func (q *Queries) UpdateAppRegistryCredential(ctx context.Context, arg UpdateAppRegistryCredentialParams) (App, error) {
	row := q.db.QueryRow(ctx, updateAppRegistryCredential, arg.ID, arg.RegistryCredentialID)
	var i App
	err := row.Scan(
		&i.ID,
		&i.ProjectID,
